...
public class MimvpProxyJava {
	...
	// 要爬取网站的用户名密码
	public static String PROXY_USERNAME = "487586385@qq.com";
	...
	public static void main(String args[]) {
		...
		// 代理无用户名密码授权
		proxy_no_auth("http", MimvpProxyJava.proxyMap.get("http"), proxyUrl); // http（proxy）
		proxy_no_auth("https", MimvpProxyJava.proxyMap.get("https"), proxyUrl2); // https（proxy）
		proxy_no_auth2("http", MimvpProxyJava.proxyMap.get("http"), proxyUrl); // http（config）
		proxy_no_auth3("http", MimvpProxyJava.proxyMap.get("http"), proxyUrl); // http（routePlanner）

		proxy_no_auth_socks("socks5", MimvpProxyJava.proxyMap.get("socks5"), proxyUrl); // socks5支持访问http网址
		proxy_no_auth_socks("socks5", MimvpProxyJava.proxyMap.get("socks5"), proxyUrl2); // socks5支持访问https网址

		// 代理需要用户名密码授权
		proxy_auth("http", MimvpProxyJava.proxyAuthMap.get("http"), proxyUrl); // http
		proxy_auth("https", MimvpProxyJava.proxyAuthMap.get("https"), proxyUrl2); // https
		proxy_auth_socks("socks5", MimvpProxyJava.proxyAuthMap.get("socks5"), proxyUrl); // socks5支持访问http网址
		proxy_auth_socks("socks5", MimvpProxyJava.proxyAuthMap.get("socks5"), proxyUrl2); // socks5支持访问https网址
	...
	public static void spider_proxy() {
		...
		try {
			...
			CloseableHttpClient client = HttpClients.createDefault(); // 推荐的用法
	...
	public static void proxy_no_auth_socks(String proxyType, String proxyIpPort, String webUrl) {
		...
		try {
			Registry reg = RegistryBuilder.create().register("http", new MyConnectionSocketFactory())
	...
	public static void proxy_auth(String proxyType, String proxyIpPort, String webUrl) {
		...
		provider.setCredentials(new AuthScope(proxy_ip, proxy_port),
				new UsernamePasswordCredentials(MimvpProxyJava.PROXY_USERNAME, MimvpProxyJava.PROXY_PASSWORD));
